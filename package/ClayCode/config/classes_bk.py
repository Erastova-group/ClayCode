...
class PathBase(ABC):
    ...
    def filter_name(
        ...
        if mode == "full":
            match =self.name.match(pattern)
        else:
            name_split = self.name.split_fname()
            if mode == "stem":
                match = name_split[0].match_str(pattern)
            elif mode == "ext":
                match = name_split[1].match_str(pattern)
            elif mode == "parts":
    ...
    def match_name(
        ...
        if mode == "full":
            ...
            name_split = self.name.split_fname()
            if mode == "stem":
                match = name_split[0].match(pattern)
            elif mode == "ext":
                match = name_split[1].match(pattern)
            elif mode == "parts":
                pattern = re.escape(pattern)
                try:
                    match = re.match(
    ...
    def match_namelist(
        ...
        if mode == "full":
            match = self.name.match(pattern_list)
        else:
            name_split = self.name.split_fname()
            if mode == "stem":
                match = name_split[0].match(pattern_list)
            elif mode == "ext":
                match = name_split[1].match(pattern_list)
            elif mode == "parts":
    ...
    def filter_namelist(
        ...
        if mode == "full":
            ...
            name_split = self.namesplit_fname()
            if mode == "stem":
                match = name_split[0].match_list(pattern_list)
            elif mode == "ext":
                match = name_split[1].match_list(pattern_list)
            elif mode == "parts":
                # pattern = re.escape(pattern_list)
                pattern = "[a-zA-Z-_\d.]*|[a-zA-Z-_\d.]*".join(pattern_list)
                match = re.match(rf"[a-zA-Z-_\d.]*{pattern}[a-zA-Z-_\d.]*", self.name)
...
print(_Path)
PathBase.register(_PosixPath)
print(_PosixPath)



class FileName(ABC):
    def split_fname(self)-> List[Union[str, None]]:
        name_split = self.split(sep=".", maxsplit=1)
        if len(name_split) > 2:
    ...
    @match.register
    def _(self, pattern: str)  -> Union[str, None]:
        if self == pattern:
            match = self
        else:
            match = None
    ...
    @match.register
    def _(self, pattern: list) -> Union[str, None]:
        pattern = "|".join(pattern)
        try:
            match = re.match(rf"{pattern}", self).group(0)
        except AttributeError:
            match = None
        return match
...
#         super().__init__(self, *args, ext="gro")
#
...
class MDPPosixFile(PosixFile, MDPFile):
    pass
...
def _get_name(name_part, ext):
    return f"{name_part.split_fname()[0]}.{ext}"
...
class ParameterBase(UserDict):
    ...
    def __getitem__(self, key, df_sel_method=lambda df: df.loc[:]):
        if key in self.kwds:
            return self.data[key]._df.df_sel_method
...
#         elif kwd in MoleculeParameter.kwds:
#             return MoleculeParameter(kwd)
...
class ITPFile(File, _Path):
    ...
    #     elif define == 1:
    #         return self._defs[kwd][definition]
    #     elif define == 2:
    ...
    def parse_itp(self):
        prm_dict = {Parameter: self._prms,#{0: self._dict[0], 1: self._defs[1], 2: self.__alts[2]},
        ...
        #             prm_dict = dicts['prms']
        #         elif kwd in MoleculeParameters.param_list:
        #             prm_dict = dicts['mols']
        #         elif kwd in SystemParameter.param_list:
        #             prm_dict = dicts['sys']
        #         else:
        ...
        #         if definition not in self._defs[kwd].keys():
        #             print("set defs t", definition)
        #             self._defs[kwd][definition] = ""  # f'#ifdef {definition}\n'
        ...
        # remove_list = []
        # for kwd in self._defs:
        #     print(kwd)
        #     for d, dd in zip(self._defs[kwd], self.__alts[kwd]):
        #         print(d, dd)
        #         if d is None:
        #             if dd is None:
    ...
    # def parse_itp(self):
    #     dicts = {'prms': {},#{0: self._dict[0], 1: self._defs[1], 2: self.__alts[2]},
