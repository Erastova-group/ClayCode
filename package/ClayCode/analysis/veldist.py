...
class VelDens(ClayAnalysisBase):
    ...
    def __init__(
        ...
        **basekwargs: Any,
        ...
        self._init_data(
            n_bins=n_bins, bin_step=bin_step, cutoff=cutoff, min=-10
        )
    ...
    def _prepare(self) -> NoReturn:
        ...
        self._3dvel_array = np.ma.empty(
            (self.sel.n_atoms, 3), dtype=np.float64, fill_value=np.nan
        )
    ...
    def _single_frame(self) -> None:
        ...
        self.data["totvel"].timeseries.append(
            np.linalg.norm(self._3dvel_array, axis=1)
        )
...
if __name__ == "__main__":
    ...
    if args.path:
        ...
        gro = path._gro  # select_file(path=path, suffix="crdin")
    ...
    # coords = outname.with_suffix("._gro")
    coords = gro
    traj = trr
    # try:
    ...
    #     sel, clay = get_selections((_gro, trr), args.sel, args.clay_type)
    ...
    sel, clay = get_selections(
        (coords, traj), args.sel, args.clay_type, in_memory=False
    )
