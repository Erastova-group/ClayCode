...
def run_analysis(
    instance: Type["ClayAnalysisBase"], start: int, stop: int, step: int
):
...
def check_traj(
    ...
    :type instance: Type['ClayAnalysisBase']
...
def process_box(instance: Type["ClayAnalysisBase"]) -> None:
...
def update_universe(f):
    def wrapper(crdname: str, crdout: Union[str, Path], **kwargs) -> Universe:
        u = Universe(str(crdname))
        f(u=u, crdout=crdout, **kwargs)
        u = Universe(str(crdout))
        return u
