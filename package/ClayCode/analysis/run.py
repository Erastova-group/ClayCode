...
if __name__ == "__main__":
    ...
    r = RunChecks(root_dir=args.root_dir, alt_dir=args.alt_root, data_df_file=args.load)
    r.run(
    ...
    if args.rsync is not None:
        try:
            ...
            r.write_complete_rsync_script(
                dest=rsync_dest,
                complete_df_json=list(Path(args.savedest).glob("complete*.json")),
                outname=Path(args.savedest) / "rsync_complete.sh",
            )
        except NotImplementedError:
            print("Invalid name for rsync destination")
    if args.fix is True:
        r.fix(odir=None, rm_tempfiles=True, overwrite=False, datpath=args.savedest)
