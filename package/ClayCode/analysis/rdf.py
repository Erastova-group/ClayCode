...
from ClayAnalysis.lib import (
    ...
    run_analysis,
    get_selections,
    save_selection, get_dist, exclude_xyz_cutoff,
)
...
class RDFDist(ClayAnalysisBase):
    ...
    def _prepare(self):
        ...
        if self.self is False:
            self._other_pos = np.empty((self.other_n_atoms, 3), dtype=np.float64)
            ...
            diag_mask = np.broadcast_to(diag_mask[..., np.newaxis], self._dist.shape)
    ...
    def _single_frame(self):
        ...
        self._sel_pos.mask = np.broadcast_to(mask[:, np.newaxis], self._sel_pos.shape)
        ...
        if self.self is False:
            ...
            self._other_pos[:] = apply_PBC(self.other.positions, self._ts.dimensions)
        else:
            new_mask = self.diag_mask + self._dist.mask
            self._dist.mask = new_mask
            self._other_pos = self._sel_pos

        get_dist(self._sel_pos, self._other_pos, self._dist, self._ts.dimensions)
        ...
        self._rad[:] = np.where(~self._dist[:,:,0].mask, np.linalg.norm(self._dist, axis=2), np.nan)

        self._rad[:] = np.where(self._rad <= self.cutoff, np.abs(self._rad), np.nan)
...
if __name__ == "__main__":
    ...
    sel, clay, other = get_selections(
        infiles=(coords, traj), sel=args.sel, other=args.other, clay_type=args.clay_type, in_memory=True
    )
