...
from ClayCode.data.consts import AA
...
class AADist(ClayAnalysisBase):
    ...
    def __init__(
        ...
        extra_sel: Optional[Union[str, List[str]]] = None,
        aas: List[str] = None,
        **basekwargs: Any,
    ) -> NoReturn:
        ...
        if aas is None:
            aas: np.array = pd.read_csv(
                AA / "aa.csv", index_col=0
            ).index.to_numpy()
            aas = np.vectorize(lambda x: x.upper())(aas)
        else:
            aas = np.array(aas)
        if extra_sel is not None:
            aas = np.append(aas, extra_sel)
        aas_str = "* ".join(aas)
        aas_str += "*"
        ...
        pos_idx = pd.MultiIndex.from_frame(
            pos_idx, names=["mol", "resnum", "atype"]
        )
    ...
    def _prepare(self):
        ...
        self._sel_df = pd.DataFrame(
            {"ag": self._sel_df, "atype": 0, "z": np.NaN}
        )
        self._sel_df.index = self._sel_df.index.reorder_levels([1, 0])
        self._dist_df = pd.DataFrame(
            index=self.pos_idx, columns=["z"], dtype=np.float64
        )
    ...
    def get_distances(self, group):
        ...
        group["atype"] = np.ravel(
            df_slice.index.get_level_values("atype").values
        )[0]
...
if __name__ == "__main__":
    parser = ArgumentParser(
        prog="aadist",
        description="Compute radial density of atom species atoms relative to other.",
        add_help=True,
        allow_abbrev=False,
    )
    parser.add_argument(
    ...
    u = MDAnalysis.Universe(
        str(coords), str(traj), format="TRR", in_memory=True
    )
