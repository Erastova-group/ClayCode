...
from ClayAnalysis.lib import save_selection, get_selections
import logging
...
parser.add_argument(
    "-o", help="Output directory.", type=str, required=False, dest="odir", default=None
)
parser.add_argument(
...
if __name__ == "__main__":
    ...
    if args.other is not None:
        ...
        sel, clay, other = get_selections(
            (tpr, trr), sel=args.sel, clay_type=args.clay_type, other=args.other
        )
        logger.info(
            f"selection: {args.sel} - {sel.n_atoms} atoms, clay: {args.clay_type} - {clay.n_atoms} atoms"
        )
        save_selection(
            outname=outname, atom_groups=[clay, sel, other], traj=[".trr", ".xtc"]
        )
    else:
        (
            sel,
            clay,
        ) = get_selections((tpr, trr), sel=args.sel, clay_type=args.clay_type)
        logger.info(
            f"selection: {args.sel} - {sel.n_atoms} atoms, clay: {args.clay_type} - {clay.n_atoms} atoms"
        )
        save_selection(
            outname=path / outname, atom_groups=[clay, sel], traj=[".trr", ".xtc"]
        )
