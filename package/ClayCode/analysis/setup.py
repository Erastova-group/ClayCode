...
from typing import Optional, List, Union, Literal, Tuple, Callable, Dict

import MDAnalysis as mda
import numpy as np

import ClayAnalysis.gmx as gmx
import ClayAnalysis.lib as lib
import ClayAnalysis.ph as ph
import ClayAnalysis.utils as ut
from ClayAnalysis import AA, FF, MDP
...
from ClayAnalysis.lib import center_clay_universe
...
def get_insert_charge(
    ...
    insert = mda.Universe(itp_path, topology_format="ITP", include_dir=include_dir)
...
def init_outpaths(
    outpath: Path, crdout: str, crdin: Union[str, Path], topin: Union[str, Path]
) -> Tuple[Path, Path]:
    """
...
def add_aa(
    ...
    # tmpdir = odir / ".tmp"
    tmpdir = TemporaryDirectory(suffix=None, prefix=".tmp", dir=odir.parent)
    ...
    logger.info(f"# RUNNING {Path(__file__).stem!r}")
    ...
    if len(list(odir.glob("*.gro"))) != 0 and overwrite is False:
        logger.info(f"Skipping {aa_str.upper()}, directory {odir} already exists.")
        ...
        if n_mols != 0:
            ...
            for aa_species in conc_dict:
                ...
                aa_gros = sorted(AA.glob(rf"pK[1-4]/{aa_species.upper()}[1-4].gro"))
                aa_itps = sorted(AA.glob(rf"pK[1-4]/{aa_species.upper()}[1-4].itp"))
                ...
                for prot_id, prot_species in enumerate(conc_dict[aa_species]):
                    ...
                    if prot_species != 0:
                        ...
                        insert_out, insert_err = gmx.run_gmx_insert_mols(
                            f=crdout,
                            ci=aa_gros[prot_id],
                            ip=posfile,
                            nmol=prot_species,
                        ...
                        if replace_check != prot_species:
                            ...
                            replaced_sol += check_insert_numbers(
                                add_repl="Replaced", searchstr=insert_err
                            )
                            logger.info(
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-odir", type=pl.Path, help="Output directory", metavar="odir", default=None
    )
    ...
    parser.add_argument(
        "-ff", type=str, metavar="force-field", help="Force field directory", default=FF
    )

    parser.add_argument(
        "-aa", type=str, nargs="+", default=None, metavar="aa", help="Amino acid(s)"
    )
    ...
    for i in ["Ca"]:  # 'Na', 'K',
        for idx in [2]:
            a = parser.parse_args(  # ala arg asn asp cys gln glu gly his ile leu lys met phe pro ser thr trp tyr val
                f"-aa asp -conc 250 -uc D2 -inp inp -pion {i} -nion Cl -ff ../data/FF -sysdir /storage/ClayAnalysis/data/CLAYS/NAu-{idx}-fe/250/{i}  -pH 7".split()
            )
            add_aa(
                aa=a.aa,
                clay_type=a.uc,
                ...
                odir=Path(
                    f"/storage/ClayAnalysis/test_data/NAu-{idx}-fe/{i}/asp_7/setup"
                ),
                pH=a.pH,
                ff_path=a.ff,
                posfile="pos.dat",
            ...
            add_aa(
                ...
                nion=a.nion,
                crdin=a.sysdir / f"{a.inp}.gro",
                odir=f"/storage/ClayAnalysis/test_data/NAu-{idx}-fe/{i}/asp_7/setup",
                pH=a.pH,
                ff_path=a.ff,
                posfile="pos.dat",
                new_outpath=False,
