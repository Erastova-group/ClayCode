...
def get_concentrations(
    ...
    for idx, e in np.ndenumerate(exp_array):
        idx = idx[0]
        c[idx] = 10.0 ** (pK_exps[idx] * np.sum(pH - pK_list[pK_start[idx] : idx + 1]))
    c[1] = c_tot / np.sum(c)
    c[mask] *= c[1]
    return np.round(c, 4)
...
def get_aa_numbers(
    ...
    if "ctl" not in aa:
        ...
        logger.debug(
            "\t"
            + 5 * " "
            + " | ".join([f"pK{n + 1}" for n in range(len(pka_df.columns) + 1)])
            + " |"
        )
        for aa in pka_df.index:
            ...
            aa_list = [f"{conc_dict[aa][n]:3.0f}" for n, nn in enumerate(conc_dict[aa])]
            # print(conc_dict)
