...
from functools import cached_property, partialmethod, singledispatch, wraps
...
import numpy as np
import pandas as pd
from ClayCode import FILE_SEARCHSTR_LIST
from ClayCode import KWD_DICT as _KWD_DICT
from ClayCode.core.utils import select_file, select_named_file
...
class UnitCell:
    def __init__(
        self,
        crds: Union[str, BasicPath],
        ff: Optional[Union[str, BasicPath]] = None,
    ):
        crds = File(crds)
        assert (
            crds.suffix == "._gro"
        ), f'Expected "._gro" file, found {crds.suffix!r}'
        self.gro = crds
...
class SimDir(Dir):
    _suffix = ""
    _suffices = ["._gro", ".top", ".trr", ".log", ".tpr"]
    _trr_sel = "largest"
    _subfolder = False
    ...
    def gro(self) -> _PosixPath:
        ...
        #     path=self.resolve(), suffix="_gro", searchlist=FILE_SEARCHSTR_LIST
        # )
    ...
    # @cached_property
    # def __pathlist = [self._gro, self.top, self.log, self.trr]
