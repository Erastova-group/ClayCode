...
from typing import Union, Optional, Literal, Tuple, List, Dict, cast, Callable, NoReturn
...
from ClayAnalysis import FF, CLAYS
...
class DirCheck:
    ...
    def __init__(
        ...
        try:
            ...
            logger.error(f"No ion numbers found for concentration of {conc} mol L-1")
            raise KeyError(f"No ion numbers found for concentration of {conc} mol L-1")
        self.reset_errors()
        self.name = name
    ...
    def system_log_charge(self) -> Union[int, None]:
        ...
        try:
            charge = re.match("[\sA-Za-z]+:\s+([-]*\d+).*", charge_str).group(1)
    ...
    def system_gro_charge(self) -> Union[int, None]:
        """Get system total charge from gro file and force field data."""
        try:
            charge = get_system_charges(crds=self.path.gro)
        except:
            charge = None
        return charge
    ...
    def _universe(self) -> Union[MDAnalysis.Universe, None]:
        ...
        try:
            ...
            try:
                logger.info("Could not find '.tpr', trying '.gro'")
                gro = str(self.path.gro.resolve())
                format = "GRO"
    ...
    def _get_ion_numbers(self, anion: str = "Cl") -> Tuple[int, int]:
        ...
        cations = re.findall(
            rf"(?<=SOL).*{self.ion}\s+(\d+)\n", topstr, flags=re.DOTALL | re.MULTILINE
        )
        anions = re.findall(
            rf"(?<=SOL).*{anion}\s+(\d+)\n", topstr, flags=re.DOTALL | re.MULTILINE
        )
    ...
    def run(self, traj_len: int, anion="Cl") -> None:
        ...
        if self.path is None:
            ...
            if charge is None and self.path.gro is not None:
                logger.info(f"Using gro charge")
            ...
            if charge is None:
                ...
                logger.error(f"2. Charge check NOT passed: " f"charge = {charge}")
                self.error_dict["charge"] = charge

            # 3. Trajectory length check
            if "trr" in self.path.suffices and (
                "tpr" in self.path.suffices or "gro" in self.path.suffices
            ):
                u = self._universe
                if u is None:
            ...
            elif "tpr" in self.path.suffices or "gro" in self.path.suffices:
                logger.error("3. Could not read trajectory.")
                # u = self._universe
            else:
                logger.error("3. Could not read trajectory or coordinates/topology.")
            ...
            if "top" in self.path.suffices:
                ...
                logger.error("4 a. Ion concentration not checked: No topology found!")
            ...
            if len(error_list) == 0:
                ...
                logger.info(f"INCOMPLETE RUN: extend simulation for {self.strinfo}\n")
                self.fixes = "extend"
            elif (
                ("gro" in missing or "top" in missing)
                ...
                logger.info(f"NO RUN: start simulation run for {self.strinfo}\n")
                self.fixes = "run"
            elif "analysis" in error_list:
                self.fixes = "analysis"
                logger.info(f"UNEXPECTED ERROR: check failed for {self.strinfo}\n")
        ...
        def bulk_ion_additions_wrapper(self, anion: str = "Cl"):
            ...
            if len(extra_ion_matches) >= 2:
                ...
                for match in extra_ion_matches:
                    extra.append(re.match(r"[A-Za-z]+\s+[0-9]+", match).group(0))
...
class RunChecks:
        ...
        if df is not None or data_df_file is not None:
            ...
            if df is not None:
                if type(df) != pd.DataFrame:
                    logger.error(f"df: Expected pd.DataFrame, found {type(df)}")
                else:
                    self.df.update(df)
                    self.new = False
            ...
            root_idx = self.df.index.get_level_values("root").astype(str).unique()
            if str(self.root_dir) not in root_idx:
                raise ValueError("Wrong root directory specified")
            if len(root_idx) == 2:
                self.alt_dir = [root for root in root_idx if root != self.root_dir][0]
            elif len(root_idx) > 2:
                raise IndexError(
        ...
        if self.new is True:
            for path_parts in self.idx_iter:
                for dir in ["orig", *self.new_dirs]:
                    ...
                    idsl = pd.IndexSlice
                    checks = DirCheck(
                        name="b", path=inpath / f"{clay}/{ion}/{aa}", subfolder=dirstr
                    )
                    if checks.path is not None:
                        ...
                        for err_key, err_val in checks.error_dict.items():
                            if err_val is not False:
                                err_val = True
                            self.df.loc[tuple([*path_parts, dir]), err_key] = err_val
                        if checks.path is not None:
    ...
    def fix(
        ...
        # apply fixes
            ...
            for run in fix_df.iterrows():
                ...
                if fix == "extend":
                    gro = Path(cols[".gro"])
                    top = Path(cols[".top"])
                    fixed_df.loc[slice, [".gro", ".top", "path", "fixes"]] = (
                        gro.resolve(),
                        top.resolve(),
                        path,
                        fix,
                ...
                # create new setup
                    ...
                    # print(fix)
                    while "setup" in str(path.resolve()).split("/") or "neutral" in str(
                        path.resolve()
                    ).split("/"):
                        path = path.parent
                    if not path.is_dir():
                        ...
                        try:
                            setup_root = path.relative_to(Path(f"{clay}/{ion}/{aa}_7"))
                        except ValueError:
                            setup_root = path
                        ...
                        i_conc = re.sub(r"([0-9]*)[.]([0-9]*)", r"\2", f"{conc}")
                        if len(i_conc) > 3:
                            raise ValueError(
                        ...
                        crdin = CLAYS / f"{clay}/{i_conc}/{ion}/inp.gro"
                        # check and fix interlayer solvent/residue numbers
                        fix_gro_residues(crdin=crdin, crdout=crdin)
                        assert Path(crdin).exists(), f"{Path(crdin)} does not exist"
        ...
        self.fixed_df = fixed_df.dropna(subset=[".gro", ".top"]).loc[
            pd.IndexSlice[:], [".gro", ".top", "path", "fixes"]
        ]
        # save applied fixes
        for fix in self.fixed_df["fixes"].unique():
            if len(self.fixed_df["fixes"] == fix) != 0:
                self.write_fixed(sel="paths", which=fix, outname=None, odir=datpath)
                self.write_fixed(sel="files", which=fix, outname=None, odir=datpath)
        ...
        for res_column in [
            ...
            if len(notna_results[res_column]) != 0:
                for sel in ["paths", "files"]:
                    self.write_results(sel=sel, which=res_column, odir=savedest)
    ...
    def add_path(self, df, drop_index=False):
        ...
        if len(df) != 0:
            ...
            vec_path_join = np.vectorize(
                lambda path, root, new: str((Path(path) / f"{root}/{new}").resolve())
            )
            ...
            if drop_index is True:
                ...
                if df.index.has_duplicates:
                    ...
                    duplicates_group = duplicates_df.groupby(duplicates_df.index)
                    for duplicate in duplicates_group:
                        for id_val in ["root", "paths"]:
                            ...
                            if nunique != 1:
                                key = grouped_df[
                                    grouped_df[id_val].apply(lambda x: idx_enum[str(x)])
                                    == np.max(
    ...
    def get_results(self, save: Union[Literal[False], str, Path] = False):
        ...
        self.results.loc[err_idx, "error"] = self.results.loc[err_idx, "error"].fillna(
            value=True
        )
        self.results.loc[err_idx, :] = self.results.loc[err_idx, :].fillna(value=False)
        if save is not False:
            savename = self.__get_savename(save, suffix=".csv")
            self.results.to_csv(str(savename.resolve()))
            logger.info(f"Writing results DataFrame to {str(savename.resolve())}")
    ...
    def write_fixed(
        ...
        if self.fixed_df is None:
            ...
            col_dict = {"paths": "path", "files": [".gro", ".top", "path"]}
            write_dict = {
                "paths": cast(
            ...
            if len(write_slice) != 0:
                write_slice = write_slice.loc[pd.IndexSlice[:], col_dict[sel]].copy()
    ...
    def completed(self) -> pd.DataFrame:
        ...
        exists_sel = self.df.loc[self.df.index.intersection(other), self.errors]
    ...
    def write_complete_rsync_script(
        ...
        rsync_head = RunChecks.sub_parts(r"(--array)=ARRAY", rf"\1={array}", rsync_head)
        ...
        for c, v in df.iterrows():
            if ci % split == 0:
                ...
                outscript.write(
                    "\nelif [[ ${SLURM_ARRAY_TASK_ID} -eq" + f" {n_task} ]]; then\n"
                )
            v.pop("path")
    ...
    def get_submit_script_str(
        ...
        script_str = self.sub_parts(
            r"(-J\s*) RUNNAME", rf"\1 {claynum}-{ion}-{aa_codes[aa]}", script_str
        )
    ...
    def write_fixed_rsync_script(
        ...
        if len(df) != 0:
            ...
            outname = Path(outname).with_suffix(".sh")
            rsync_name = outname.with_stem(outname.stem + "_rsync").with_suffix(".sh")
            outscript = open(outname, "w+")
            ...
            for idx, cols in df.iterrows():
                ...
                try:
                    searchpath = f"{dest.parent}/{gro.relative_to(root_dir).parent}"
                    rel_gro = gro.relative_to(root_dir)
                    rel_top = top.relative_to(root_dir)
                except ValueError:
                    searchpath = (
                        f"{dest.parent}/{gro.relative_to(self.root_dir).parent}"
                    )
                ...
                if fix == "extend":
                    tpr = self.results[
                        self.results[".gro"].astype(str) == str(gro.resolve())
                    ][".tpr"]
                ...
                if out.stdout != "":
                    ...
                    outscript.write(f"sbatch {dest.parent}/{rel_dir}/{script_name}\n")
                    rsync_script.write(
            ...
            outscript.close()
            rsync_script.close()
            logger.info(f"Wrote fixed rsync script to {rsync_name.resolve()!r}")
            logger.info(f"Wrote fixed run submit_script to {outname.resolve()!r}")
            logger.info(f"Submit script destination: '{remote}:{dest.resolve()}'")
...
if __name__ == "__main__":
    ...
    r.run(
        traj_len=35001, savedest="/storage/ClayAnalysis/results/runcheck_results_local/"
    )
