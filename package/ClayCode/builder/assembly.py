...
class Builder:
    ...
    def solvate_clay_sheets(self) -> None:
        ...
        solvent: Solvent = Solvent(x_dim=self.sheet.dimensions[0],
    ...
    def rename_il_solv(self) -> None:
        ...
        il_resnames: list = list(map(lambda resname: re.sub('SOL', 'iSL', resname), il_resnames))
    ...
    def select_atoms_outside_clay(self, atomgroup: AtomGroup, extra: Union[int, float]=0) -> AtomGroup:
        atom_group: AtomGroup = select_outside_clay_stack(atom_group=atomgroup,
                                  clay=self.clay,
                                  extra=extra)
    ...
    def add_bulk_ions(self) -> None:
        if self.extended_box is True:
            ...
            for ion, values in ion_df.iterrows():
                charge, conc = values
                n_ions: int = np.rint(bulk_z * bulk_x * bulk_y * constants['N_Avogadro'] * conc * 1E-27).astype(int) # 1 mol/L = 10^-27 mol/A
    ...
    def stack_sheets(self) -> None:
        ...
        for sheet_id in range(self.args.n_sheets):
            ...
            if il_solv is not False:
                ...
                sheet_u.dimensions[2]: float = sheet_u.dimensions[2] + 1
            else:
    ...
    def get_filename(self, *solv_ion_args, suffix=None, sheetnum: Optional[int]=None, tcb_spec=None) -> Union[GROFile, TOPFile]:
        ...
        logger.debug(f'{self.__tmp_outpath.name} exists: {Path(self.__tmp_outpath.name).is_dir()}')
        # self.__tempfile = tempfile.NamedTemporaryFile(dir=outpath, delete=False)
        path: Union[TOPFile, GROFile] = FileFactory(f'{self.__tmp_outpath.name}/{fstem}{suffix}')
        return path
    ...
    def __path_setter_copy(self, property_name: str, file: Union[Path, str, GROFile]) -> None:
        ...
        if path is not None:
            ...
            try:
                ...
                logger.debug(logger.debug(f'Copied {path.top.name} to {self.args.outpath.name}'))
                ...
                for ion, n_ions in self.args.n_il_ions.items():
                    if n_ions != 0:
                        ...
                        with tempfile.NamedTemporaryFile(suffix='.gro') as ion_gro:
                            ion_u: Universe = Universe.empty(n_atoms=1, n_residues=1, n_segments=1,
...
class Sheet:
    ...
    def write_gro(self) -> None:
        ...
        uc_n_atoms: NDArray = np.array([self.uc_data.n_atoms[uc_id] for uc_id in self.uc_array]).reshape(self.x_cells, self.y_cells)
        x_repeats: Callable = lambda n_atoms: self.__cells_shift(n_atoms=n_atoms, n_cells=self.x_cells)
        y_repeats: Callable = lambda n_atoms: self.__cells_shift(n_atoms=n_atoms, n_cells=self.y_cells)
        x_pos_shift: NDArray = np.ravel(np.apply_along_axis(x_repeats, arr=uc_n_atoms, axis=0), order='F')
        y_pos_shift: NDArray = np.ravel(np.apply_along_axis(y_repeats, arr=uc_n_atoms, axis=1), order='F')
...
class Solvent:
    ...
    def __init__(self, x_dim: Optional[Union[int, float]]=None,
                 y_dim: Optional[Union[int, float]]=None,
                 z_dim: Optional[Union[int, float]]=None, n_mols: Optional[Union[int]]=None,
    ...
    def get_sheet_solvent_mols(self, z_dim: Union[float, int]) -> int:
        mols_sol = (z_dim / 10 * constants['N_Avogadro'] * self.x_dim / 10 * self.y_dim / 10 * self.solv_density) / (
            self.mw_sol)
    ...
    def write(self, spc_name: GROFile, topology: Optional[TopologyConstructorBase] = None) -> None:
        ...
        solv, out = run_gmx_solvate(cs='spc216', maxsol=self.n_mols,
                                  o=spc_gro, p=spc_top, scale=0.00010, v="",
    ...
    def check_solvent_nummols(self, solvate_stderr: str) -> None:
        added_wat: str = re.search(r'(?<=Number of solvent molecules:)\s+(\d+)',
